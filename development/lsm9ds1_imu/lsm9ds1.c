...
/*
 * Initial standalone test. When the driver is integrated into the application,
 * move this main() into a separate main.c.
 */
int main(int argc, char **argv)
{
    const char *device = (argc > 1) ? argv[1] : "/dev/i2c-1";
    const struct timespec display_delay = {
        .tv_sec = 0,
    ...
    if (lsm9ds1_init(fd) < 0) {
        fprintf(stderr, "LSM9DS1 initialization failed: %s\n",
                strerror(errno));
        lsm9ds1_close(fd);
        return 1;
    }
    ...
    for (i = 0; i < 100; ++i) {
        ...
        if (lsm9ds1_read_sample(fd, &sample) < 0) {
            fprintf(stderr, "Sensor read failed: %s\n", strerror(errno));
            lsm9ds1_close(fd);
            return 1;
        }

        total_g = sqrtf(sample.accel_g.x * sample.accel_g.x +
                        sample.accel_g.y * sample.accel_g.y +
                        sample.accel_g.z * sample.accel_g.z);

        printf("ACCEL raw=(%6d, %6d, %6d)  "
               "g=(%7.3f, %7.3f, %7.3f) total=%6.3f g\n",
               sample.accel_raw.x, sample.accel_raw.y, sample.accel_raw.z,
               sample.accel_g.x, sample.accel_g.y, sample.accel_g.z, total_g);

        printf("GYRO  raw=(%6d, %6d, %6d)  "
               "rad/s=(%7.3f, %7.3f, %7.3f)\n\n",
               sample.gyro_raw.x, sample.gyro_raw.y, sample.gyro_raw.z,
               sample.gyro_rad_s.x, sample.gyro_rad_s.y,
               sample.gyro_rad_s.z);

        nanosleep(&display_delay, NULL);
    }
